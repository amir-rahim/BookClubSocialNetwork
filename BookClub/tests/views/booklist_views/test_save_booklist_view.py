"""Unit testing of the Save Booklist view"""
...
class SaveBookListViewTestcase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('save_booklist', kwargs={'booklist_id': self.booklist_to_save.id})

    def test_save_booklist_url(self):
        self.assertEqual(self.url, f'/library/lists/{self.booklist_to_save.id}/save/')
    ...
    def test_get_save_booklist_redirects_to_user_booklist(self):
        ...
        redirect_url = reverse('user_booklist',
                               kwargs={'booklist_id': self.booklist_to_save.id})
    ...
    def test_cannot_save_own_booklist(self):
        ...
        url = reverse('save_booklist', kwargs={'booklist_id': own_booklist.id})
        self.client.login(username=self.user.username, password='Password123')
        before_count = self.user.get_saved_booklists().count()
