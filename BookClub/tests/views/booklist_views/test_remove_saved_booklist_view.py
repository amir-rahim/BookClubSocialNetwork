...
class RemoveSavedBookListViewTestcase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('remove_saved_booklist', kwargs={'username': self.creator.username, 'booklist_id': self.booklist_to_save.id})
        
    def test_save_booklist_url(self):
        self.assertEqual(self.url, f'/user/{self.creator}/lists/{self.booklist_to_save.id}/remove_saved_booklist/')
    ...
    def test_cannot_remove_from_others_saved_booklists(self):
        ...
        url = reverse('remove_saved_booklist', kwargs={'username': self.creator.username, 'booklist_id': own_booklist.id})
        self.client.login(username=self.user.username, password='Password123')
