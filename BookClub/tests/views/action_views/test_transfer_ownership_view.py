...
from BookClub.models import User, Club, ClubMembership, user
from django.contrib import messages
from BookClub.tests.helpers import LogInTester
from django.core.exceptions import ObjectDoesNotExist

from BookClub.views import membership
...
class PromoteMemberView(TestCase, LogInTester):
    ...
    def test_owner_transfer_to_moderator(self):
        ...
        self.assertTrue(ClubMembership.objects.filter(user=self.moderator,club=self.club,membership=ClubMembership.UserRoles.OWNER).exists())
        self.assertTrue(ClubMembership.objects.filter(user=self.owner,club=self.club,membership=ClubMembership.UserRoles.MODERATOR).exists())
    ...
    def test_owner_transfer_to_wrong_moderator(self):
        currentUser = self.owner
        """Using applicant here as they are the moderator of club 2"""
        targetUser = self.applicant
        ...
        self.assertFalse(ClubMembership.objects.get(user=currentUser, club=self.club).club == ClubMembership.objects.get(user=targetUser, club=self.club2).club)
        """Checking the ranks before"""
        current_user_rank_before = ClubMembership.objects.get(user=currentUser, club=self.club).membership
        self.assertEqual(current_user_rank_before,ClubMembership.UserRoles.OWNER)
