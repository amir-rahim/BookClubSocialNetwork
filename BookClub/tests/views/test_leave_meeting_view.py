...
class LeaveMeetingViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('leave_meeting', kwargs={'url_name' : self.club.club_url_name, 'meeting_id': self.future_meeting.id})

    def test_url(self):
        self.assertEqual(self.url,f'club/{self.club.url_name}/meetings/{self.future_meeting.id}/join')
    ...
    def test_get_leave_meeting_redirects_to_list_of_meetings(self):
        ...
        response = self.client.get(reverse('leave_meeting', kwargs={'url_name' : self.club.club_url_name, 'meeting_id': self.future_meeting.id}))
    ...
    def test_member_successful_leave_meeting(self):
        ...
        response_message = self.client.get(reverse('leave_meeting'))
    ...
    def test_member_leave_meeting_not_in(self):
        ...
        self.assertFalse(self.future_meeting.get_members().filter(username = self.user.username).exists())
        response_message = self.client.get(reverse('leave_meeting', kwargs={'url_name' : self.club.club_url_name, 'meeting_id': self.future_meeting.id}))
        messages_list = list(response_message.context['messages']) 
        self.assertEqual(len(messages_list), 1)
    ...
    def test_member_cannot_leave_meeting_in_past(self):
        ...
        self.assertTrue(self._is_logged_in())
        ClubMembership.objects.create(user=self.user, club=self.club, membership=ClubMembership.UserRoles.MEMBER)
        self.past_meeting.join_member(self.user)
        ...
        response_message = self.client.get(reverse('leave_meeting', kwargs={'url_name' : self.club.club_url_name, 'meeting_id': self.past_meeting.id}))
        messages_list = list(response_message.context['messages']) 
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(messages_list[0].level, messages.INFO)
