...
class MeetingDetailsViewTestCase(TestCase, LogInTester):
    ...
    def setUp(self):
        ...
        self.url = reverse('meeting_details', kwargs={'club_url_name' : self.club.club_url_name, 'meeting_id' : self.meeting.id})
        #self.url = '/club/'+self.club.club_url_name+'/meetings/'+str(self.meeting.id)+'/'
    @tag('url')
    def test_url(self):
        #self.assertEqual(self.url,f'/club/{self.club.club_url_name}/meetings/{self.meeting.id}')
    ...
    def test_can_see_meeting_name_and_club(self):
    ...
    def test_can_see_meeting_details_book_meeting(self):
        self.client.login(username=self.user.username, password="Password123")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "meeting_details.html")
    ...
    def test_can_see_details_social_meeting(self):
        self.client.login(username=self.user.username, password="Password123")
        social_meeting = Meeting.objects.get(pk='5')
        social_meeting_url = reverse('meeting_details', kwargs={'club_url_name' : self.club.club_url_name, 'meeting_id' : social_meeting.id})
    ...
    def test_can_see_details_club_meeting(self):
        self.client.login(username=self.user.username, password="Password123")
        club_meeting = Meeting.objects.get(pk='6')
        club_meeting_url = reverse('meeting_details', kwargs={'club_url_name' : self.club.club_url_name, 'meeting_id' : club_meeting.id})
    ...
    def test_can_see_details_other_meeting(self):
        self.client.login(username=self.user.username, password="Password123")
        other_meeting = Meeting.objects.get(pk='7')
        other_meeting_url = reverse('meeting_details', kwargs={'club_url_name' : self.club.club_url_name, 'meeting_id' : other_meeting.id})
    ...
    def test_can_see_organiser_info(self):
        self.client.login(username=self.user.username, password="Password123")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "meeting_details.html")
