"""Unit testing of the Book Review Detail view"""
...
@tag("views", "review", "book_review")
class BookReviewViewTestCase(TestCase, LogInTester):
    """Tests for Book Review Detail view"""
    fixtures = [
    ...
    def setUp(self):
        ...
        self.main_review_url = reverse("book_review",
                                       kwargs={"book_id": self.book_review.book_id, "review_id": self.book_review.id})
        self.other_review_url = reverse("book_review", kwargs={"book_id": self.another_book_review.book_id,
                                                               "review_id": self.another_book_review.id})
        self.no_comment_review_url = reverse("book_review", kwargs={"book_id": self.no_comment_book_review.book_id,
                                                                    "review_id": self.no_comment_book_review.id})

    def test_book_review_detail_view_url(self):
        self.assertEqual(self.main_review_url,
                         f'/library/books/{self.book_review.book.id}/review/{self.book_review.id}/')
        self.assertEqual(self.other_review_url,
                         f'/library/books/{self.another_book_review.book.id}/review/{self.another_book_review.id}/')
    ...
    def guest_see_no_comments_on_commentless_review(self):
        ...
        self.assertContains(response,
                            "There are no comments for this review. Comment using the reply button on the review.")
