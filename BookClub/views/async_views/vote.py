...
class CreateVoteView(LoginRequiredMixin, CreateView):
    """ Creates a Vote, returns a JSONResponse representing the updated rating and which button the JS front end should highlight"""
    model = Vote
    http_method_names = ['post']
    form_class = VoteForm

    def post(self, request, *args, **kwargs):
        """Checks if the user has already voted on the current comment, post or review. If they have, custom code handles the replacement of the existing vote with a new one."""
        form = self.get_form()
        if form.is_valid():
    ...
    def get_response_json(self, form, vote):
        """Checks what type of vote has been case and builds the correct JSONResponse containing vote type and updated rating for target object"""
        try:
